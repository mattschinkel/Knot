...
- Evaluated ChatGPT's AIR language options for an LLM-native programming language. Best core: typed expression tree as the write format, stable node IDs so edits are graph operations, structured compiler errors/holes. Defer agents, knowledge graphs, uncertainty, and intent-compilation until the kernel exists.
...
- **Autonomy flip (2026-09-09): R1 decides everything; no human gate.** Per the author, the core invariant changed from "R1 drafts, human decides" to "R1 decides autonomously; human observes via the dashboard (non-blocking)." R1 now owns all design decisions AND all git operations (including reset/force-push, with a backup first; R6 logs them). Updated everywhere: `golem_agents.md` (§1 principle, §4 escalation→decides, §6 git rule, §7 R1 role/goal/backstory), `ai_docs/ai.txt` (git rule), `phase0_spec.md`/`phase1_spec.md` ("human confirms"→"R1 decides"), `README.md`, dashboard. `architect_phase.py` R1 prompt now tells R1 to DECIDE (not draft-for-human) and answer its own Qs. Mitigation: R4 stays a hard correctness gate; R6 logs every decision so nothing is silent.
- **Drift-check phase gate (the proper fix for autonomous taste).** Made the 10/10 LLM-syntax bar **measurable, not a vibe** so a 4B can catch taste drift without a human backstop. New `ai_docs/golem_agents.md` §8 defines 5 metrics — M1 token count (lower better, prereq Phase 1 parser), M2 generation accuracy under GBNF (higher, Phase 1), M3 edit round-trip (higher, Phase 1), M4 partial-compile coherence (higher, Phase 4), M5 compile latency (lower, Phase 9) — run at every phase boundary by R3+R4 via `src/golem/drift.py:check()`, compared to `docs/drift_baseline.json`; any regression is flagged to R1 and the phase does NOT close until the gate passes (PENDING metrics don't fail it). Implemented `src/golem/drift.py` (deterministic, no LLM/IO; `Status`/`Metric`/`MetricResult`/`DriftReport` + `check()` + `summarize()`) + `tests/test_drift.py` (10 tests). `golem_design.md` §2 now states the bar is measurable. `architect_phase.py --tasks N` always appends the drift gate as the final task. Dashboard shows a drift card. **53 tests pass.** This is proper (the metrics are exactly the qualities the design already demands), not a hack.
- Git rule change: "never `git checkout HEAD` without the **Architect's (R1) OK**" (was: without the author's OK) — the gate is delegated to R1, who may escalate to the human. Recorded in `ai_docs/golem_agents.md` §6 R6.3 and `ai_docs/ai.txt`.
...
- Added GitHub remote `origin` and pushed `master` (tracks `origin/master`). Language is Golem locally; renaming the GitHub repo to `mattschinkel/Golem` is a follow-up. Agents push normal commits to `origin`; no force-push/reset without R1's OK + backup.
- Name search concluded 2026-09-12: locked **Golem** + `.gol` + `golemlang.com`.
...
- **Phase 2 T4 landed (2026-09-09).** Crew imported nonexistent `TypeErrorVal` from `values`. Manual `type_error`/`TypeErrorVal` in `checker.py` (errors-as-values). Harness now rejects `from golem.<mod> import Missing`. Resuming `--from=T5`.
...
- **Phase 2 T10 landed manually (2026-09-10).** Crew invented `golem.expr` / wrong CondExpr. Manual `infer_if` for IfExpr + IF/COND OpExpr (BOOL cond, unify branches). Resuming `--from=T11`.
...
- **Phase 2 T12 landed manually (2026-09-10).** Crew invented `golem.expr`. Manual `infer_fn` + `FnType`; CallExpr arity/arg unify. Resuming `--from=T13`.
- **Design-doc AIR examples rewritten (2026-09-10).** `ai_docs/golem_design.md` leftover `def` / juxtaposition-`FN` / spaced AIR in §5–§15 and §21 now match D-FB1/D-FB2/D-FB11 (`DEF[...]`, `FN[[params],body]`, no-space canonical). Pretty-view examples stay human sugar. Phase 3 contracts/effects noted as not-yet-locked AIR.
- **Phase 2 T13 landed manually (2026-09-10).** Crew mangled `__init__.py` + invented `golem.nodes`. Restored package init; `infer_def` binds DefNode name→body type in Env. Resuming `--from=T14`.
...
- **Phase 2 DONE (2026-09-10).** T17 property tests (`test_no_raise`, `test_hole_ok`); removed bogus `src/golem/checker` file from crew; T18 drift gate PASS vs updated `docs/drift_baseline.json` (M1=21). **367 tests.**
- **Autobuild auto-restart (2026-09-10).** Failed tasks retry (`GOLEM_TASK_RETRIES`) and outer-loop restart (`GOLEM_AUTO_RESTART=1`); `--watch` re-execs on process exit. See `fixes/fix_autobuild_auto_restart.md`. Continuing Phase 3.
...
- **Crew abandoned; Cursor drives Golem (2026-09-11).** 4B crew not used for implementation going forward. Phase 3 T5–T12 landed manually: contracts, FnType effects/caps, infer_fn annotations, validate_call_effects, props, drift gate. **414 tests.** Baseline `docs/drift_baseline.json` phase 3.
...
- **Phase 9 DONE (2026-09-12).** Bytecode VM (`src/golem/vm/`): compile AST → chunk, CALL/RETURN, hole trap; `measure_m5` <200ms. **523 tests.**
...
- **Phase 12 DONE (2026-09-12).** MCP tools (eval/typecheck/run/constrain/doc_query/query); `kb` token budget; `--llm` policies; `python -m golem`. **587 tests. §16 build complete.**
- **Self-host bootstrap (2026-09-12).** Stage 0.5–2 + phased full features: A1 surface (holes/ERR/typed/RECORD), A2 IMPORT/`root.gol`, A3 `check.gol`, A4 Stage-2 gates, B `golemc_bridge` + `golem compile`, C pretty-view/PAR opcodes/borrow/live MODEL handler.
...
- Rename GitHub `origin` to `mattschinkel/Golem` when ready (local tree already Golem / `.gol`).
...
- Prefer Stage-1 golemc for deterministic lowering — DONE (`golemc_bridge`).
- Compile-all deterministic AIR via golemc — DONE (`fixes/fix_compile_all_golem.md` + finish).
...
- Stage 0.5 / Stage 1 expr golemc / Stage 2 expr gate — DONE 2026-09-12.
...
- Start Phase 0 of Golem: define value representation, type representation, and units/dimensions (see `ai_docs/golem_design.md`).
- Decide node ID scheme (author-pinned vs compiler-assigned) — RESOLVED in `ai_docs/golem_design.md` §21. Decision: LLM addresses nodes by structural path + optional symbolic labels; numeric IDs are compiler-internal, never authored in source. `@` introduces a label, not a numeric ID.
- Competitive scan (Phase 0.5): DONE — all six projects verified real. Findings + must-adopt features in `ai_docs/golem_design.md` §20. Top adopts: GBNF grammar for constrained decoding, compiler-as-MCP-server, token-budgeted `kb` tool, `--llm` output mode, sub-200ms compile, errors-as-values, two-half compiler, prohibitions-as-constraints.
- Finish wish-list review (see `ai_docs/golem_design.md` §19): DONE — must-have features from competitive scan folded into Tier 1 (#11-14) and Tier 2 (#22-29). Build phases updated (§16) to include GBNF grammar (Phase 1), errors-as-values + two-half compiler (Phase 5/9), compiler-as-MCP + kb + --llm mode (Phase 12).
- Decide which Tier 3 items make v1, and which rejected items stay rejected.
- Answer the 4 open questions in `ai_docs/golem_agents.md` §5 — RESOLVED 2026-09-08: (1) Phase 0 host language = **Python** (Rust deferred to Phase 9 if sub-200ms forces it); (2) R1 Architect = **peer** drafting tasks out-of-band (not a hierarchical manager — 4B model loses coherence managing live); (3) shell tool = **gated behind human approval** in Phase 0 (agents propose, human runs); (4) repo layout = **confirmed** + add `vm/` (bytecode target) and `docs/` (extracted spec tables).
...
- **Phase 0 spec drafted** (`ai_docs/phase0_spec.md`) — R1's deliverable: value repr (`values.py`), type algebra (`types.py`), units/dimensions (`units.py`), structured errors (`errors.py`), with 6 design decisions (D1–D6) and 3 open sub-questions (Q1–Q3) for the human. Repo skeleton created (`src/golem/`, `src/golem/ai/`, `tests/`, `cli/`, `lsp/`, `fmt/`, `grammar/`, `vm/`, `docs/`). **Awaiting human review of D1–D6 + Q1–Q3 before R2 writes implementation code.**
...
- Specify the Golem string stdlib (slice/index/length/char<->int) — flagged "NOT yet specified" in `ai_docs/golem_design.md` §22.5; add to Phase 0/stdlib.
...
- Language name: **Golem** LOCKED 2026-09-12. Register `golemlang.com` + USPTO. Extension `.gol` (known GeoDesk/GOL overlap — accepted).

- END GOAL: self-hosting (Golem compiles its own compiler). Bootstrap plan in `ai_docs/golem_design.md` §22. Stage 0 in Python (recommendation) targeting a bytecode VM; bootstrap the deterministic lowering compiler in Golem first; add the AI front-end (ai-effectful) after self-hosting; native (LLVM/C) backend later. Language must add: string stdlib, bytecode VM target, `exec` capability (native backend only).
...
- **NEW: deterministic spec-consistency lint for the phase gate — DONE.** Built `tools/spec_lint.py` (deterministic, no LLM/IO) + `tests/test_spec_lint.py` (12 tests). Catches the class of error the drift gate (M1–M5) cannot: spec-level contradictions. Rules: R1 status (autonomous, not stale/"for human review"/"implemented; tests green" with unchecked DoD), R2 numbering (strictly increasing `## N.` from 1 — catches the §0 restart), R3 DoD boxes (unchecked in drafts), R4 invariants (no `D` may assert numeric IDs appear in GBNF/source/emitted syntax — §21; canonical form is bracket notation), R5 required sections. Wired into the phase gate (`golem_agents.md` §8 step 1, runs before drift metrics) and into `architect_phase.py --draft N` (auto-lints + gives R1 one repair pass). Proven: the fixed `phase1_spec.md` passes; a reconstruction of R1's original broken draft fails with exactly the defects R1 produced (numbering §4→§0, INV-A contradiction). **65 tests pass.**
...
- Stage 0.5/1/2 self-host — `runtime_ops`, DEF/FN Stage-1 golemc, Stage 2 self-compile round-trip.
...
- `fix_phased_full_features.md` — A1–C: golemc surface/modules/check, golemc_bridge, pretty/PAR/borrow/MODEL HTTP.
- `fix_compile_all_golem.md` — Stage-1 golemc lowers full deterministic AIR (COND/PAR/SEQ/REF/stubs/quotes).
- `fix_compile_all_finish.md` — compile_root IMPORT link, granted_caps in _call_def, INVOKE stub.
- `fix_golemc_cli_rebuild.md` — golem run/compile/rebuild; rebuild_roundtrip + CALL_PROGRAM.
- Phase 12 — MCP tools, kb, --llm formatter, `python -m golem`.
...
- T22 bin clash — crew created both `src/golem/bin.py` and `src/golem/bin/`; fixed to module + AST JSON round-trip tests.
...
- Phase 2 T17 crew fail — wrote bogus `src/golem/checker` (not .py) and never created prop tests; landed T17+T18 manually and closed Phase 2.
...
- Phase 2 T13 crew fail — mangled `__init__.py` / `golem.nodes`; landed `infer_def` manually.
- Phase 2 T12 crew fail — invented `golem.expr`; landed `infer_fn` + `FnType` manually.
- Phase 2 T11 crew fail — Env.get_type / TypeVar fantasy + raises; landed `infer_hole` + `ANY` manually.
- Phase 2 T10 crew fail — invented `golem.expr` / CondExpr; landed `infer_if` for IfExpr + IF/COND OpExpr manually.
...
- `src/golem/vm/` — bytecode compiler + stack VM. `from golem.vm import run, measure_m5`
- `src/golem/ai_ffi.py` — MODEL/TOOL registry, invoke, constrain. `from golem.ai_ffi import ToolRegistry, invoke`
- `src/golem/concurrency.py` — PAR/SEQ/REF/UNSAFE. `from golem.concurrency import eval_par, check_unsafe`
- `python -m golem` — CLI: `mcp`, `kb`, `llm`, `tool` (Phase 12). Example: `PYTHONPATH=src python -m golem tool eval --args '{"source":"ADD[1,2]"}'`
- `src/golem/mcp/` — MCP tools + stdio JSON-RPC (no HTTPS). `from golem.mcp import call_tool`
- `src/golem/kb.py` / `llm_out.py` — token-budgeted kb + `--llm` policies
- `src/golem/runtime_ops.py` — Stage 0.5 string/list/map/record/MATCH/IO ops
- `selfhost/` — Stage 1 golemc (`.gol`) + harness (`harness/test_stage1.py`, `test_stage2.py`)
