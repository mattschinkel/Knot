# fix: finish compile-all — IMPORT link + caps + INVOKE stub

## Problem
Compile-all still needed in-Golem multi-file IMPORT linking. Nested DEF
evaluation dropped `granted_caps`, so `FS_READ` inside `compile_root` always
failed. INVOKE had no compile-time stub. Dashboard/docs still showed self-host
as incomplete.

## Fix
- `partial._call_def`: thread `granted_caps` into nested DEF evaluation.
- `main.gol`: `compile_root[dir,root]` expands IMPORT via `FS_READ` + merge.
- `codegen.gol`: INVOKE → `ERR_MAKE['invoke_host_only']` (host AI runtime).
- Fixtures `link_a.gol` / `link_root.gol`; gates in `test_compile_all_golem.py`.
- `golemc_bridge` uses `resolve_stage1_imports()`; dashboard/selfhost_plan/progress
  mark compile-all + IMPORT link done.

## Host-only (by design)
MCP, surgical edits, live MODEL/INVOKE tool execution remain Python.
