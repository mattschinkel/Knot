# Golem — AI-native programming language (design)

Author: Matthew Schinkel
Status: design / v0.1

NOTE ON THE NAME (LOCKED 2026-09-12): The language is **Golem**.
Source files use the **`.gol`** extension. Public site/package branding
uses **golemlang** (`golemlang.com` — register before public release).

Etymology: a golem is a construct animated by inscription — models
author the canonical AIR; humans use the pretty view. The name signals
AI-native / not human-primary source.

Known bare-name collisions (mitigated by golemlang branding):
- Golem Network (decentralized compute; GLM token) — avoid `.glm` ext
- Golem Cloud (durable agent runtime) — adjacent AI niche
- PyPI/npm packages named `golem` — prefer distribute as `golemlang`
- Several small GitHub “Golem language” toys — none dominant

File extension **`.gol`** is also used by GeoDesk Geo-Object Libraries
and a small Go-Oriented Lisp; accepted by author lock.
...
- **The 10/10 bar is measurable, not a vibe.** Taste drift is caught by a
  drift-check phase gate (see `ai_docs/golem_agents.md` §8) with concrete
...
### 2.1 Does Golem require an LLM? No.

An LLM is NOT required to compile or run a Golem program. The kernel is
...
So: a pure Golem program compiles and runs with zero LLM calls. The LLM is
...
- Golem Network / Golem Cloud — bare “Golem” already used in compute
  marketplace and durable-agent-runtime products; we brand as Golemlang
  for site/package while the language name remains Golem.
...
END GOAL: Golem's compiler is written in Golem and Golem compiles
itself. This section drives several design decisions.

### 22.1 Why Golem is well-suited to self-host
The compiler's job is to manipulate the AST/IR as data. In Golem, the
...
a compiler in Golem is more natural than in most languages.

### 22.2 Bootstrap stages (T-diagram)
- Stage 0 (scaffolding, thrown away): a minimal Golem compiler
  ...
  NOT need to be fast.
- Stage 1: write the Golem compiler itself in Golem (a subset first).
  Compile it with Stage 0. Now we have a Golem compiler produced by
  Stage 0.
- Stage 2: the Stage-1 compiler compiles itself -> self-hosting.
  Stage 0 (Python) is retired. From here on, Golem is built by Golem.
...
compiler in Golem first — no LLM dependency to build the compiler.
...
Most of it is already in the design. To express a compiler, Golem needs:
...
- After Phase 9 + enough stdlib: write the compiler-in-Golem (Stage 1).
