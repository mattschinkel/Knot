"""Drift-check phase gate for the 10/10 LLM-syntax bar (design ┬º2, agents ┬º8).
...
not a failure ΓÇö so the gate is structurally ready and fills in as the
compiler grows (parser Phase 1, GBNF Phase 1, partial-compile Phase 4,
VM Phase 9).

No LLM, no parser, no I/O here ΓÇö this is deterministic kernel code that R4
...
def _is_regression(metric: Metric, current: float, baseline: float | None) -> bool:
    ...
    No baseline yet (first run) is NOT a regression ΓÇö it establishes the baseline.
    """
    if baseline is None:
        return False
