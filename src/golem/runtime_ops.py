...
def eval_op(
    ...
    *,
    ...
    if op == "CALL_PROGRAM":
        # CALL_PROGRAM[program, entry_name, args_list] — run a golemc Program image.
        if len(args) != 3:
            return _err("arity", "CALL_PROGRAM", "expected program, name, args")
        prog, name_v, args_v = args[0], args[1], args[2]
        from golem.golemc_bridge import _program_from_sum
        from golem.vm.machine import VM

        if not isinstance(prog, SumVal) or prog.tag != "Program":
            return _err("type", "CALL_PROGRAM", "expected Program sum")
        if not isinstance(name_v, StringVal):
            return _err("type", "CALL_PROGRAM", "entry name must be string")
        if not isinstance(args_v, ListVal):
            return _err("type", "CALL_PROGRAM", "args must be list")
        img = _program_from_sum(prog)
        if isinstance(img, ErrorVal):
            return SumVal("Err", StringVal(str(img.err.message)))
        try:
            result = VM(img, granted_caps=granted).call(
                name_v.value, list(args_v.items)
            )
        except Exception as e:
            return SumVal("Err", StringVal(str(e)))
        if isinstance(result, ErrorVal):
            return SumVal("Err", StringVal(str(result.err.message)))
        return result
