"""Gate: knotc (Stage-1) compiles remaining deterministic AIR forms."""
...
def test_knotc_compile_all_forms(stage1_prog, src, check):
    assert check(_run(stage1_prog, src))


def test_knotc_semicolon_skipped(stage1_prog):
    assert _run(stage1_prog, ";ADD[1,2];") == I(3)


def test_knotc_invoke_compiles_host_stub(stage1_prog):
...
def test_compile_root_links_fixture(stage1_prog):
    ...
    r = _call_def_values(
        stage1_prog,
        "compile_root",
        [StringVal(str(FIXTURES)), StringVal("link_root.knot")],
        granted_caps=caps,
    )
    assert isinstance(r, SumVal) and r.tag == "Program"
...
def test_vm_compile_root_stage1():
    """In-Knot IMPORT link of full Stage-1 via VM compile_root."""
    from knot.effects import Capability, CapabilitySet
    from selfhost.harness.compile_stage1 import STAGE1
    from selfhost.harness.test_stage2 import _host_knotc
    from knot.vm.machine import VM

    caps = CapabilitySet([Capability.FS_READ])
    vm = VM(_host_knotc().image, granted_caps=caps)
    r = vm.call("compile_root", [StringVal(str(STAGE1)), StringVal("root.knot")])
    assert isinstance(r, SumVal) and r.tag == "Program"
    img = program_from_sum(r)
